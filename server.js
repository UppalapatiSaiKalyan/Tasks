...
const { add, subtract, multiply, divide } = require('./arithmetic_ope');
...
app.get('/sub', authenticateToken, (req, res) => {
  const { a, b } = req.query;
  const numA = parseFloat(a);
  const numB = parseFloat(b);
...
app.get('/mul', authenticateToken, (req, res) => {
  const { a, b } = req.query;
  const numA = parseFloat(a);
  const numB = parseFloat(b);
...
app.get('/div', authenticateToken, (req, res) => {
  const { a, b } = req.query;
  const numA = parseFloat(a);
  const numB = parseFloat(b);
